...
if [ -d *"homeproxy"* ]; then
	...
	git clone -q --depth=1 --single-branch --branch "release" "https://github.com/Loyalsoldier/surge-rules.git" ./surge
	cd ./surge && RES_VER=$(git log -1 --pretty=format:'%s' | grep -o "[0-9]*")
